...
function paramsToQueries(searchParams) {
  console.log("paramsToQueries:", JSON.stringify(Array.from(searchParams)));
...
export const createOverviewSlice = (set, get) => ({
  ...
  initialize: async (repoRoute, search) => {
    ...
    console.log("overview_slice: initialize", searchParams, search);
    let repoUUID;
  ...
  updateOverview: async () => {
    ...
    console.log("store/overview_slice.js: updateOverview", queries);
    const api = new API(repoUUID);
  ...
  onQueries: async () => {
    console.log("store/overview_slice.js: onQueries");

    if (get().isInitialized) {
      const { queries, repoName } = get();
      console.log("store/overview_slice.js: onQueries", queries);
